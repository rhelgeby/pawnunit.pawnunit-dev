...
DOCS_DEST=$RELEASEDIR/doc/pawnunitdemo
...
# Rebuild vcsversion.inc for bundled source.
...
then
    ...
    cp -r $GAMEDIR/* $RELEASEDIR
else
    # Make diff files with config changes for each patch.
    sh diffgen.sh
    
    # Copy only changed files.
    CHANGEDFILES=$(hg status --rev $PATCHREV | grep $GAMEDIR/ | cut -d ' ' -f2 | cut -d '/' -f2-)
