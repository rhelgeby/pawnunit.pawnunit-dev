...
PLUGININCLUDES=src/include
BUILDDIR=build
SPCOMP_LINUX=env/linux/bin/spcomp-1.4.4
SPCOMP_DARWIN=env/darwin/bin/spcomp-1.4.4
...
%.smx: %.sp
	$(VERSIONDUMP)
	$(SPCOMP) -i$(SOURCEDIR) -i$(SMINCLUDES) -i$(PLUGININCLUDES) -o$(BUILDDIR)/$@ $<
